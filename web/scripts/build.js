...
// Warn and crash if required files are missing if
...
checkBrowsers(paths.appPath, isInteractive).then(() => {
  // First, read the current file sizes in build directory. This lets us display
  // how much they changed later.
  return measureFileSizesBeforeBuild(paths.appBuild);
}).then(previousFileSizes => {
  // Remove all content but keep the directory so that if you're in it, you don't
  // end up in Trash fs.emptyDirSync(paths.appBuild+'/static'); //别删文件啊啊啊啊啊啊 Merge
  ...
  return new Promise((resolve, reject) => {
    compiler.run((err, stats) => {
      ...
      if (messages.errors.length) {
        ...
        // but confuse the reader with noise.
