...
module.exports = function (proxy, allowedHost) {
  return {
    ...
    // https://medium.com/webpack/webpack-dev-server-middleware-security-issues-1489d
    // 950874a However, it made several existing use cases such as development in
    // cloud environment or subdomains in development significantly more
    // complicated: https://github.com/facebook/create-react-app/issues/2271
    // https://github.com/facebook/create-react-app/issues/2233 While we're
    // investigating better solutions, for now we will take a compromise. Since our
    // WDS configuration only serves files in the `public` folder we won't consider
    // accessing them a vulnerability. However, if you use the `proxy` feature, it
    // gets more dangerous because it can expose remote code execution
    // vulnerabilities in backends like Django and Rails. So we will disable the
    // host check normally, but enable it if you have specified the `proxy` setting.
    // Finally, we let you override it if you really know what you're doing with a
    ...
    // `index.html`, you can get URL of `public` folder with %PUBLIC_URL%: <link
    ...
    // listening to the compiler events with `compiler.hooks[...].tap` calls above.
    quiet: true,
    // Reportedly, this avoids CPU overload on some systems.
    // https://github.com/facebook/create-react-app/issues/293 src/node_modules is
    // not ignored to support absolute imports
    ...
    before(app, server) {
      ...
      // previous service worker registered for the same host:port combination. We do
      // this in development to avoid hitting the production cache if it used the same
      // host and port.
      // https://github.com/facebook/create-react-app/issues/2272#issuecomment-30283243
      // 2
